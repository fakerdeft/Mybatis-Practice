	...
	<div class="outer" align="center">
		...
		<div id="paging-area">
			<c:if test="${pi.currentPage != 1}">			
				<a href="search.bo?currentPage=${pi.currentPage - 1}&category=${category }&keyword=${keyword }">[이전]</a>
			</c:if>
			<c:forEach var="p" begin="${pi.startPage}" end="${pi.endPage}">
				<a href="search.bo?currentPage=${p}&category=${category }&keyword=${keyword }">[ ${p} ]</a>
			</c:forEach>
			<c:if test="${pi.currentPage ne pi.maxPage }">
				<a href="search.bo?currentPage=${pi.currentPage + 1}&category=${category }&keyword=${keyword }">[다음]</a>
			</c:if>
		</div>
		<br><br><br><br><br>
	</div>
